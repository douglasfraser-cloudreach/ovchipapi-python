...
class OvApi:
    ...
    def get_token(
        self, username, password, client_id=CLIENT_ID, client_secret=CLIENT_SECRET
    ):
        post_data = {
            "username": username,
            "password": password,
        ...
        r = requests.post(
            "https://login.ov-chipkaart.nl/oauth2/token", data=post_data
        ).json()
    ...
    def refresh_token(
        self, refresh_token, client_id=CLIENT_ID, client_secret=CLIENT_SECRET
    ):
        post_data = {
    ...
    def get_authorization(self):
        ...
        response = requests.post(
            "https://api2.ov-chipkaart.nl/femobilegateway/v1/api/authorize",
            data=post_data,
        )
    ...
    def get_cards_list(self, locale="nl-NL"):
        post_data = {"authorizationToken": self.authorizationToken, "locale": locale}

        response = requests.post(
            "https://api2.ov-chipkaart.nl/femobilegateway/v1/cards/list", data=post_data
        )
        as_json = response.json()
        return as_json["o"]
    ...
    def get_card(self, mediumId, locale="nl-NL"):
        ...
        response = requests.post(
            "https://api2.ov-chipkaart.nl/femobilegateway/v1/card/", data=post_data
        )
        as_json = response.json()
        return as_json["o"]
    ...
    def get_transaction_list(
        ...
        mediumId,
        offset=0,
        startDate=time.strftime("%Y-%m-01", time.gmtime()),
        ...
        response = requests.post(
            "https://api2.ov-chipkaart.nl/femobilegateway/v1/transactions",
            data=post_data,
        )
        as_json = response.json()
